...
/**
 * Migrates all attachments from a ServiceNow incident to a Freshservice ticket.
 *
 ...
 */
export async function migrateIncidentAttachments(
  snowIncidentId,
  freshTicketId,
  context
) {
  ...
  try {
    const response = await snowClient.get(`${snowBaseUrl}/api/now/attachment`, {
      table_name: 'incident',
      table_sys_id: snowIncidentId,
      sysparm_limit: 100,
    });
    attachments = response?.result ?? [];
  } catch (err) {
    logger.error(
      `[incident-attachments] Failed to fetch attachments for SN incident ${snowIncidentId}: ${err.message}`
    );
    return summary;
  }
  ...
  for (const attachment of attachments) {
    ...
    if (sizeNum > MAX_ATTACHMENT_BYTES) {
      logger.warn(
        `[incident-attachments] Skipping ${file_name} (${sizeNum} bytes) — exceeds 20MB limit (snow_sys_id=${sys_id})`
      );
      summary.skipped++;
      continue;
    }
    ...
    try {
      ...
      logger.error(
        `[incident-attachments] Failed to download ${file_name} (snow_sys_id=${sys_id}): ${err.message}`
      );
      summary.failed++;
      continue;
    }
    ...
    try {
      ...
      logger.info(
        `[incident-attachments] Migrated ${file_name} (snow_sys_id=${sys_id}) → fresh_ticket_id=${freshTicketId}`
      );
      summary.migrated++;
    } catch (err) {
      logger.error(
        `[incident-attachments] Failed to upload ${file_name} (snow_sys_id=${sys_id}): ${err.message}`
      );
      summary.failed++;
    }
  }
