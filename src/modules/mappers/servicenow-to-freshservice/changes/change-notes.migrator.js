// src/modules/mappers/servicenow-to-freshservice/changes/change-notes.migrator.js
...
/**
 * Migrates all notes/comments from a ServiceNow change to a Freshservice change.
 *
 ...
 */
export async function migrateChangeNotes(snowChangeId, freshChangeId, context) {
  ...
  try {
    const response = await snowClient.get(
      ...
      {
        sysparm_query: `element_id=${snowChangeId}^element=work_notes^ORelement=comments`,
    ...
    logger.error(
      `[change-notes] Failed to fetch notes for SN change ${snowChangeId}: ${err.message}`
    );
    return summary;
  }
  ...
  for (const note of notes) {
    ...
    if (!rawValue || !rawValue.trim() || !stripHtml(rawValue)) {
      logger.warn(
        `[change-notes] Skipping empty note snow_sys_id=${snowSysId} for change ${freshChangeId}`
      );
      summary.skipped++;
      continue;
    }
    ...
    if (!stripHtml(body)) {
      logger.warn(
        `[change-notes] Skipping note snow_sys_id=${snowSysId}: body is empty after processing`
      );
      summary.skipped++;
      continue;
    }
    ...
    // POST to /changes/{id}/notes
    try {
      const result = await throttledPost(
        fsClient,
        `/api/v2/changes/${freshChangeId}/notes`,
        notePayload,
        { tag: 'change-notes' }
      );
      const freshNoteId = result?.note?.id ?? result?.id ?? 'unknown';
      logger.info(
        `[change-notes] Migrated note snow_sys_id=${snowSysId} → fresh_note_id=${freshNoteId}`
      );
      summary.migrated++;
    } catch (err) {
      const fsError = err?.response?.data;
      logger.error(
        `[change-notes] Failed to post note snow_sys_id=${snowSysId} to change ${freshChangeId}: ${err.message} | FS response: ${JSON.stringify(fsError)}`
      );
      summary.failed++;
    }
  }
